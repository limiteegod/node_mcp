...
IssueModel.prototype.check = function(msg, dTerm, cb)
{
    ...
    async.waterfall([
        ...
        //遍历每一条记录
        //update 处理模式 1.先处理通知消息 2. 批量更新数据 3. 最后更新未中奖在所有队列处理完成,也就是反奖的时候处理
                    ...
                    table.findAndRemove({}, {}, [], function(err, ticket){
                        ...
                        if(ticket)
                        {
                            ...
                            self.sendMsg(ticket, bonusInfo, function(err, data){
                                ...
                                //中奖 将SQl语句暂时放入内存;
                                if(ticket.bonus > 0 ){
                                    ticketSer.newHit(ticket, function(err, data){
                                        sql += data;
                                        sqlCount ++ ;
                                        if(sqlCount == 1000){
                                            var start = new Date().getTime();
                                            mainConn.execute(sql, {}, function(err, data){
                                                var end = new Date().getTime();
                                                log.info("批处理更新1000条耗时" + (end-start)+"ms");
                                                cb(err, data);
                                            });
                                            sqlCount = 0;
                                            sql = "";
                        ...
                        else
                        {
                            if(sql.length > 0){
                                ...
                                mainConn.execute(sql, {}, function(err, data){
                                    var end = new Date().getTime();
                                    log.info("处理不足1000条结束"+(end-start)+"ms");
                                    cb(err, data);
                                });
                            }
                            hasNext = false;
