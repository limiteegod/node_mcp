 ...
 * @param cb
 */
IssueModel.prototype.check = function(msg, dTerm, dir, cb)
{
    var self = this;
            ...
            async.whilst(
                ...
                function (err) {
                    writeData.end(function(){
                        ticketSer.saveHit(file, function(){
...
IssueModel.prototype.prize = function(msg, dTerm, cb)
{
    ...
    async.waterfall([
        //跟新中奖
        function (cb) {
            var sql = "UPDATE tticket a INNER JOIN hitticket b on  a.id = b.id SET a.bonus = b.bonus,a.bonusDetail = b.bonusDetail ,a.bonusBeforeTax=b.bonusBeforeTax,a.auditTermCode=b.auditTermCode,a.dNumber=b.dNumber,a.status=b.status,a.prizeTime="+now;
            var start = new Date().getTime();
            conn.execute(sql, {}, function (err, data) {
                log.error("更新中奖执行sql时间"+(new Date().getTime() - start));
                cb(err);
            })
        },
        //删除临时数据
        function (cb) {
            var sql = "truncate table hitticket";
            conn.execute(sql, {}, function (err, data) {
                cb(err);
            })
        },
