...
IssueModel.prototype.check = function(msg, dTerm, cb)
{
    ...
    var cursor = table.find({}, {}, {snapshot:true}).sort({_id:1});
    async.waterfall([
        //获得奖级
        function(cb)
        {
        ...
        function(check, count, cb)
        {
            ...
            async.whilst(
                ...
                function (err) {
                    cb(err, count, curCount, curTicketId);
                }
            );
        }
    ], function (err, total, count, curTicketId) {
        log.info(colName + ",算奖结束,总记录数:" + total + ",完成数:" + count + ",票据号:" + curTicketId);
        cursor.close();
        cb(err, {total:total, finished:count, flag:curTicketId});
    });
}
