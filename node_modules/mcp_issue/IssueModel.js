...
IssueModel.prototype.check = function(msg, dTerm, cb)
{
    ...
    dir = path.join(dir,dTerm.gameCode,currDate,dTerm.code);
    ...
    async.waterfall([
        ...
        function(check, count, cb)
        {
            ...
            var curCount = 0;
            var hasNext = true;
            var file = path.join(dir,colName+".txt");
            var tableName = dTerm.gameCode+dTerm.code;
            var writeData = fs.createWriteStream(file);
            var buf = new Array();
            async.whilst(
                function () { return hasNext;},
                function (callback) {
                        ...
                        if(ticket)
                        {
                            ...
                            self.sendMsg(ticket, bonusInfo, function(err, data){
                                ...
                                if(ticket.bonus > 0){
                                    ...
                                    var split = "@";
                                    strB.append(ticket.id).append(split).append(ticket.outerId).append(split).append(ticket.bonus).append(split).append(JSON.stringify(ticket.bonusDetail)|| '""').append(split).append(ticket.bonusBeforeTax).append(split).append(ticket.auditTermCode).append(split).append(ticket.dNumber).append(split).append(ticketStatus.HIT);
                                    buf.push(strB.toString());
                                    if(buf.length == 1000){
                                        writeData.write(buf.join("\n"));
                                        writeData.write("\n");
                                        buf = [];
                                    }
                                    callback(null);
                ...
                function (err) {
                    writeData.end(function(){
                        ticketSer.saveHit(file, tableName, function(){
...
IssueModel.prototype.prize = function(msg, dTerm, cb)
{
    ...
    async.waterfall([
        //跟新中奖
        function (cb) {
            var sql = "UPDATE tticket a INNER JOIN " + tableName + " b on  a.id = b.id SET a.bonus = b.bonus,a.bonusDetail = b.bonusDetail ,a.bonusBeforeTax=b.bonusBeforeTax,a.auditTermCode=b.auditTermCode,a.dNumber=b.dNumber,a.status=b.status,a.prizeTime="+now;
            log.info(sql);
            var start = new Date().getTime();
            conn.execute(sql, {}, function (err, data) {
                if(err)
                {
                    log.error("返奖(更新票据数据)失败................");
                    log.error(err);
                    cb(err);
                }
                else
                {
                    log.info("返奖(更新票据数据)成功................");
                    log.info("更新中奖执行sql时间"+(new Date().getTime() - start));
                    cb(null);
                }
            })
        },
        ...
        function (cb) {
            //更新數據庫
            ticketSer.newNotHit(dTerm, function(err, data){
                cb(err);
            });
        },
        //销售角度统计中奖数目
        ...
        function(cb)
        {
            var sql = "select customerId,count(*) as saleCount,sum(amount) as saleAmount from tticket where printStatus=1300  and " + cond + " group by customerId";
            conn.execute(sql, {}, function(err, data){
                for(var key in data)
                {
                    var set = data[key];
                    ...
                    obj.customerId = set.customerId;
                    obj.saleCount = set.saleCount;
                    obj.saleAmount = set.saleAmount;
                }
                cb(err);
            });
        },
        ...
        function(cb)
        {
            var sql = "select printId as customerId,count(*) as saleCount,sum(amount) as saleAmount from tticket where printStatus=1300  and " + cond + " group by printId";
            conn.execute(sql, {}, function(err, data){
                for(var key in data)
                {
                    var set = data[key];
        ...
        //查询当前游戏的所有销售渠道
        function(cb){
            var cond = {gameCode:dTerm.gameCode};
            var sort = {customerId:1};
            var relationTable = dc.mg.get("relation");
            relationTable.find(cond,{customerId:1}).sort(sort).toArray(function (err, data) {
                if (data) {
                    for(var key in data)
                    {
                        var item = data[key];
                        var key = item.customerId + "_" + termReportType.SALE;
                        if(rst[key] == undefined){
                            obj = {type:termReportType.SALE, createTime:now};
                            rst[key] = obj;
        ...
        function(cb){
            ...
            var sort = {relayTo:1};
            var relationTable = dc.mg.get("relation");
            relationTable.find(cond,{relayTo:1}).sort(sort).toArray(function (err, data) {
                if (data) {
                    for(var key in data)
                    {
                        var item = data[key];
                        var key = item.relayTo + "_" + termReportType.PRINT;
                        if(rst[key] == undefined){
                            obj = {type:termReportType.PRINT, createTime:now};
                            rst[key] = obj;
                            obj.customerId = item.relayTo;
                        }
                    }
