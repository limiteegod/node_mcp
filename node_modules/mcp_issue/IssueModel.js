    ...
    async.waterfall([
        ...
        function(cb){
            ...
            relationTable.find(cond,{customerId:1}).sort(sort).toArray(function (err, data) {
                if (data) {
                    log.info("chaxun data __________________________");
                    log.info(data);
                    var key = data.customerId + "_" + termReportType.SALE;
                    if(rst[key] == undefined){
                        obj = {type:termReportType.SALE, createTime:now};
                        rst[key] = obj;
        ...
        function(cb){
            ...
            relationTable.find(cond,{relayTo:1}).sort(sort).toArray(function (err, data) {
                if (data) {
                    log.info("chaata __________________________");
                    log.info(data);
                    var key = data.relayTo + "_" + termReportType.PRINT;
                    if(rst[key] == undefined){
                        obj = {type:termReportType.PRINT, createTime:now};
                        rst[key] = obj;
                        obj.customerId = data.relayTo;
                    }
                }
                cb(err);
            });
