...
Check::Check()
{
    this->gl = NULL;
    this->pDrawNum = NULL;
    this->bounsRule = {{5,2,1}, {5,1,2}, {5,0,3}, {4,2,3}, {4,1,4}, {4,0,5}, {3,2,4}, {3,1,5}, {2,2,5}, {3,0,6}, {1,2,6}, {2,1,6}, {0,2,6}};
}
...
Handle<Value> Check::Count0000(const Arguments& args)
{
    ...
    int len = 0 ; //每个单式的长度
    char *lotNumChar = (char *)malloc(sizeof(char *)*40);
    for(int i = 0; i < length ; i++ ){
        char temp = pChar[i];
        lotNumChar[len] = temp; //copy 值
        if(temp == ';' || i == length -1 ){ //单式结束或者真正结束了
            len ++;
            lotNumChar[len] = '\0';
            //要算将的num
            ...
            lotNumChar = (char *)malloc(sizeof(char *)*40);
            len = 0;
            continue;
        }
        len ++;
