    ...
    if(self->pDrawNumType->getType() == NUM_TYPE_Z12)
    {
        ...
        for(int j = 0; j < length; j++)
        {
            ...
            if(jChar == ';' || j == length - 1) //号码结束
            {
                ...
                if(hit)
                {
                    long curBonus = self->gl->getBonus(1);
                    long curBonusBeforeTax = curBonus;
                    Handle<Object> obj = Object::New();
                    obj->Set(String::NewSymbol("bonus"), Number::New(curBonus));
                    obj->Set(String::NewSymbol("bonusBeforeTax"), Number::New(curBonusBeforeTax));
                    obj->Set(String::NewSymbol("level"), Number::New(2));
                    obj->Set(String::NewSymbol("count"), Number::New(1));
                    array->Set(array->Length(), obj);
                    bonus += curBonus;
                    bonusBeforeTax += curBonusBeforeTax;
...
Handle<Value> Check::Count0201(const Arguments& args)
{
    ...
    if(self->pDrawNumType->getType() == NUM_TYPE_Z12)
    {
        ...
        {
            long curBonus = self->gl->getBonus(1);
            long curBonusBeforeTax = self->gl->getBonus(1);
            Handle<Object> obj = Object::New();
            obj->Set(String::NewSymbol("bonus"), Number::New(curBonus));
            obj->Set(String::NewSymbol("bonusBeforeTax"), Number::New(curBonusBeforeTax));
            obj->Set(String::NewSymbol("level"), Number::New(2));
...
Handle<Value> Check::Count0202(const Arguments& args)
{
    ...
    if(self->pDrawNumType->getType() == NUM_TYPE_Z12)
    {
        ...
        if(hit)
        {
            ...
            if(tuoHitCount + danLen < 3)    //总hit数必须大于等于4
            {
                hit = false;
            }
        }
