...
CheckFileJob.prototype._createTempTable = function(cb)
{
    ...
    var sql = "create table  " + self.tmpHitTable + " as select * from hitticket";
    var conn = dc.main.getConn();
    conn.execute(sql, {}, function (err, data) {
        if(err)
        {
        ...
        function (callback) {
            hitTable.findAndRemove({}, {}, [], function(err, ticket){
                ...
                if(ticket)
                {
                    ...
                    var split = "@";
                    strB.append(ticket._id).append(split).append(ticket.outerId).append(split).append(ticket.bonus).append(split).append(JSON.stringify(ticket.bonusDetail)|| '""').append(split).append(ticket.bonusBeforeTax).append(split).append(ticket.auditTermCode).append(split).append(ticket.dNumber).append(split).append(ticketStatus.HIT);
        ...
        function (err) {
            ...
            else
            {
                writeData.end(function(){
                    ticketSer.saveHit(self.file, self.tmpHitTable, function(err, data){
                        cb(err, curCount);
                    });
                });
            }
