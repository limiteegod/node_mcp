...
Handle<Value> Check::Count0501(const Arguments& args)
{
    ...
    for(int i = 0; i < numArray->length() ; i ++){
        ...
        firstArray->put(first);
        secondArray->put(second);
...
Handle<Value> Check::Count0800(const Arguments& args)
{
    ...
    for(int i = 0; i < prizeArray->length()-1; i++){
       int temp = prizeArray->get(i);
       int index = 0;
       for( int j = i +1; j<prizeArray->length() && prizeArray->get(j) < prizeArray->get(i); j++){
          //插入最小的值到最前面。
          index = j;
          prizeArray->modify(i, prizeArray->get(j));
       }
       prizeArray->modify(index, temp);
    }
    printf("排序后:%d",prizeArray->get(0));
    printf("排序后:%d",prizeArray->get(1));
    printf("排序后:%d",prizeArray->get(2));
