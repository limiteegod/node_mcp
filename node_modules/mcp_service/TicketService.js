...
TicketService.prototype.printFail = function(ticket, auditTermCode, cb)
{
    ...
    var table = dc.main.get("tticket");
    var cond = {id:ticket.id};
    var data = {$set:{printStatus:ticketPrintStatus.PRINT_FAILURE,
    auditTermCode:auditTermCode, status:ticketStatus.REFUND}};
    table.update(cond, data, [], function(err, data){
        cb(err, data);
    });
}
...
{
    var table = dc.main.get("tticket");
    var cond = {printStatus:ticketPrintStatus.PRINT_SUCCESS,status:ticketStatus.UN_OPEN,termCode:dTerm.code,gameCode:dTerm.gameCode};
    var data = {$set:{status:ticketStatus.NOT_HIT,
        dNumber:dTerm.wNum, auditTermCode:dTerm.code}};
    table.update(cond, data, {}, function(err, data){
        if(cb)
        {
            cb(err, data);
...
TicketService.prototype.saveHit = function(file, cb){
    var conn = dc.main.getConn();
    var star = new Date().getTime();
    var sql="load data local infile '"+file+"' replace into table hitticket FIELDS TERMINATED BY '@' LINES TERMINATED BY '\n' (id,bonus,bonusDetail,bonusBeforeTax,auditTermCode,dNumber,status)" ;
    conn.execute(sql, {} ,function(err ,data){
        log.error("load 耗时" + (new Date().getTime() - star));
        log.error(err);
        cb(err);
    });
}
