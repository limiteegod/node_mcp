...
var accountRole = cons.accountRole;
...
SalePageControl.prototype.money = function (headNode, bodyNode, cb) {
    ...
    //页面显示科目查询项
    var roleId=accountRole[headNode.userType];
    var role = ac.getInfoById(roleId);
    backBodyNode.accountType=role.accounts[0].types;
    backBodyNode.accountIncome=backBodyNode.accountType[0].subjects;
    backBodyNode.accountExpense=backBodyNode.accountType[1].subjects;
    var cursor = table.find(backBodyNode.cond, {}, []).sort(backBodyNode.sort).limit(backBodyNode.skip, backBodyNode.limit);
    cursor.toArray(function(err, data){
        for(var key in data)
        {
