...
ChannelTradeControl.prototype.handleCT03 = function(user, headNode, bodyNode, cb)
{
    ...
    async.waterfall([
        ...
        {
            order.id = kv.value;
            moneySer.handle(customerId, order.id, "channel", "cash", "out", "lot", order.amount, function(err, data){
                log.info(data);
                cb(err);
            });
        },
        ...
        function(order, tickets, cb)
        {
            ...
            async.each(tickets, function(ticket, callback) {
                var pTicket = {_id:ticket.id, gameCode:ticket.gameCode, termCode:ticket.termCode,
                pType:ticket.pType, bType:ticket.bType, printId:ticket.printId, multiple:ticket.multiple,
                amount:ticket.amount, number:ticket.number, version:0, status:ticketPrintQueenStatus.WAITING_GET};
